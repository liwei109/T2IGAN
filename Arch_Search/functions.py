...
def train_shared(args, gen_net: nn.Module, dis_net: nn.Module, g_loss_history, d_loss_history, controller, gen_optimizer
    ...
    noise = Variable(torch.FloatTensor(args.train_batchsize, 100))
    noise = noise.cuda()
    # eval mode
    controller.eval()
    for epoch in range(args.shared_epoch):
        ...
        while iter_idx < len(train_loader)//6:  # 8855/batchsize
            data = data_iter.next()
            imgs, captions, cap_lens, class_ids, keys = prepare_data(data)

            # Adversarial ground truths
            real_imgs = imgs[controller.cur_stage].type(torch.cuda.FloatTensor)  ### imgsize 64 128 256
            ...
            # z = torch.cuda.FloatTensor(np.random.normal(0, 1, (imgs[0].shape[0], args.latent_dim)))
            ...
            fake_imgs = gen_net(sent_embs, noise).detach()
            ...
            if step % args.n_critic == 0:
                ...
                gen_imgs = gen_net(sent_embs, noise)
...
def train_controller(args, controller, ctrl_optimizer, gen_net, test_loader, text_encoder, prev_hiddens, prev_archs, writer_dict):
    ...
    for step in range(args.ctrl_step):
        ...
        for arch in archs:
            ...
            is_score = get_is(args, gen_net, test_loader, text_encoder)
...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, text_encoder,
    ...
    while iter_idx < len(train_loader):  # 8855/batchsize
        global_steps = writer_dict['train_global_steps']
        data = data_iter.next()
        imgs, captions, cap_lens, class_ids, keys = prepare_data(data)

        # Adversarial ground truths
        real_imgs = imgs[2].type(torch.cuda.FloatTensor)  ### imgsize 64 128 256
    ...
    #     z = torch.cuda.FloatTensor(np.random.normal(0, 1, (imgs.shape[0], args.latent_dim)))
        ...
        fake_imgs = gen_net(sent_embs).detach()
...
def get_is(args, gen_net: nn.Module, dataloader, text_encoder):
    """
    Get inception score.
    :param args:
    :param gen_net:
    ...
    noise = Variable(torch.FloatTensor(args.test_batchsize, 100))
    noise = noise.cuda()
    ...
    while iter_idx < len(dataloader)//2:  # 293
        data = data_iter.next()
        imgs, captions, cap_lens, class_ids, keys = prepare_data(data)
    ...
    #     z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.eval_batch_size, args.latent_dim)))
        ...
        noise.data.normal_(0,1)
        gen_imgs = gen_net(sent_embs, noise).mul_(127.5).add_(127.5).clamp_(0.0, 255.0).permute(0, 2, 3, 1).to('cpu',
                                                                                                torch.uint8).numpy()
...
def validate(args, test_loader, text_encoder, fid_stat, gen_net: nn.Module, writer_dict, clean_dir=True):
    writer = writer_dict['writer']
    ...
    # generate images
    fixed_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.test_batchsize, args.latent_dim)))
    ...
    while iter_idx < len(test_loader):  # 293
        data = data_iter.next()
        imgs, captions, cap_lens, class_ids, keys = prepare_data(data)
    ...
    #     z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.eval_batch_size, args.latent_dim)))
        ...
        # Generate a batch of images
        gen_imgs = gen_net(sent_embs).mul_(127.5).add_(127.5).clamp_(0.0, 255.0).permute(0, 2, 3, 1).to('cpu',
                                                                                                torch.uint8).numpy()
...
def get_topk_arch_hidden(args, controller, gen_net, dataloader, text_encoder, prev_archs, prev_hiddens):
    """
    ~
    :param args:
    :param controller:
    ...
    for arch_idx in range(len(archs)):
        ...
        is_score = get_is(args, gen_net, dataloader, text_encoder)
        logger.info(f'get Inception score of {is_score}')
