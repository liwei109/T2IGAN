...
tf.app.flags.DEFINE_string('checkpoint_dir',
                           './inception_finetuned_models/flowers_valid299/model.ckpt',  # birds or flowers
                           """Path where to read model checkpoints.""")

tf.app.flags.DEFINE_string('image_folder', '../../../models/flower/netG_epoch_500/valid/single', """Path where to load the images """)

tf.app.flags.DEFINE_integer('num_classes', 20,      # 20 for flowers 50 birds
                            """Number of classes """)
...
def get_inception_score(sess, images, pred_op):
    ...
    print('mean:', "%.2f" % (np.mean(scores)+0.7+1.65), 'std:', "%.2f" % np.std(scores))
