...
class INIT_STAGE_G(nn.Module):
    def __init__(self, ngf, ncf, arch):
        super(INIT_STAGE_G, self).__init__()
        self.gf_dim = ngf # 32x16
    ...
    def define_module(self):
        ...
        # stage 1
        self.cell1 = Cell(self.gf_dim, self.gf_dim//2, num_skip_in=0, hw=4)
        self.cell2 = Cell(self.gf_dim//2, self.gf_dim//4, num_skip_in=1, hw=8)
        self.cell3 = Cell(self.gf_dim//4, self.gf_dim//8, num_skip_in=2, hw=16)
        self.cell4 = Cell(self.gf_dim//8, self.gf_dim//16, num_skip_in=3, hw=32)
    ...
    def set_arch(self, arch_id):
        if not isinstance(arch_id, list):
            arch_id = arch_id.split(", ")
            # arch_id = arch_id.to('cpu').numpy().tolist()
        arch_id = [int(x) for x in arch_id]

        arch_cell1 = arch_id[:4]
        self.cell1.set_arch(conv_id=arch_cell1[0], norm_id=arch_cell1[1], up_id=arch_cell1[2],
                            short_cut_id=arch_cell1[3], skip_ins=[])

        arch_cell2 = arch_id[4:9]
        self.cell2.set_arch(conv_id=arch_cell2[0], norm_id=arch_cell2[1], up_id=arch_cell2[2],
                                short_cut_id=arch_cell2[3], skip_ins=arch_cell2[4])

        arch_cell3 = arch_id[9:14]
        self.cell3.set_arch(conv_id=arch_cell3[0], norm_id=arch_cell3[1], up_id=arch_cell3[2],
                                short_cut_id=arch_cell3[3], skip_ins=arch_cell3[4])
        
        arch_cell4 = arch_id[14:]
        self.cell4.set_arch(conv_id=arch_cell4[0], norm_id=arch_cell4[1], up_id=arch_cell4[2],
                                short_cut_id=arch_cell4[3], skip_ins=arch_cell4[4])
    ...
    def forward(self, z_code, c_code):
        ...
        h1_skip_out, out_code8 = self.cell1(out_code4)
        h2_skip_out, out_code16 = self.cell2(out_code8, (h1_skip_out,))
        h3_skip_out, out_code32 = self.cell3(out_code16, (h1_skip_out, h2_skip_out))
        _, out_code64 = self.cell4(out_code32, (h1_skip_out, h2_skip_out, h3_skip_out))
        # c_z_code = torch.cat((c_code, z_code), 1)
        # # state size ngf x 4 x 4
        # out_code = self.fc(c_z_code)
...
# class NEXT_STAGE_G(nn.Module):
#     def __init__(self, ngf, nef, ncf, hw, arch):
#         super(NEXT_STAGE_G, self).__init__()
#         self.gf_dim = ngf
#         self.ef_dim = nef
#         self.cf_dim = ncf # 100
...
#         self.residual = self._make_layer(ResBlock, ngf * 2)
#         self.upsample = upBlock(ngf * 2, ngf)
#         # self.Transblock = Cell(ngf*2, ngf, num_skip_in=0, hw=self.hw)
...
#         self.Transblock.set_arch(conv_id=arch[0], norm_id=arch[1], up_id=arch[2],
#                             short_cut_id=arch[3], skip_ins=[])
...
#         # # state size ngf/2 x 2in_size x 2in_size
#         out_code = self.upsample(out_code)
#         # out_code = self.Transblock(h_c_code)
...
class G_NET(nn.Module):
    def __init__(self):
        ...
        if cfg.TREE.BRANCH_NUM > 0:
            self.h_net1 = INIT_STAGE_G(ngf * 16, ncf, cfg.arch1)
            self.img_net1 = GET_IMAGE_G(ngf)
        # gf x 64 x 64
        if cfg.TREE.BRANCH_NUM > 1:
            self.h_net2 = NEXT_STAGE_G(ngf, nef, ncf, 64) # , cfg.arch2)
            self.img_net2 = GET_IMAGE_G(ngf)
        if cfg.TREE.BRANCH_NUM > 2:
            self.h_net3 = NEXT_STAGE_G(ngf, nef, ncf, 128) # , cfg.arch3)
